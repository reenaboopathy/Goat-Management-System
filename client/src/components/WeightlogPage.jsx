...
/* =========================================================
   STORAGE
   NOTE:
   Milk records are NOT stored in localStorage.
   Only tenant/goat information may still come from
...
async function getRealGoats() {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch goats. Server returned ${response.status}`
      );
    }

    const data = await response.json();

    const goats = Array.isArray(data)
      ? data
...
/* =========================================================
   GET MILK RECORDS FROM BACKEND
========================================================= */

async function getMilkRecordsFromBackend() {
  const response = await fetch(
    `${API_BASE}/milk-records`,
    {
      credentials: "include",
    }
  );
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch milk records. Server returned ${response.status}`
    );
  }
...
async function createMilkRecordInBackend(
  record
) {
  const response = await fetch(
    `${API_BASE}/milk-records`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `Failed to save milk record. Server returned ${response.status}`
    );
  }
...
async function updateMilkRecordInBackend(
  id,
  record
) {
  const response = await fetch(
    `${API_BASE}/milk-records/${id}`,
    {
      method: "PUT",
      credentials: "include",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `Failed to update milk record. Server returned ${response.status}`
    );
  }
...
async function deleteMilkRecordFromBackend(
  id
) {
  const response = await fetch(
    `${API_BASE}/milk-records/${id}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `Failed to delete milk record. Server returned ${response.status}`
    );
  }
...
export default function WeightlogPage({
  ...
  /*
    IMPORTANT:
    Milk records start empty.
    They are loaded from MongoDB using API.
  */
    ...
    const loadData =
      async () => {
        try {
          ...
          const milkRecords =
            await getMilkRecordsFromBackend();
          ...
          window.alert(
            "Unable to load milk records from backend. Please make sure the backend server is running."
          );
        } finally {
          setLoading(false);
        }
  ...
  const handleSave = async (
    ...
    try {
      ...
      if (exists) {
        result =
          await updateMilkRecordInBackend(
            entry.id,
            entry
          );
      } else {
        result =
          await createMilkRecordInBackend(
            entry
          );
      }
      ...
      if (!savedRecord) {
        throw new Error(
          "Backend returned an invalid milk record."
        );
      }
  ...
  const handleDelete =
    async (id) => {
      ...
      try {
        ...
        await deleteMilkRecordFromBackend(
          id
        );
