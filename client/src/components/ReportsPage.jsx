...
/* =========================================================
   GET ALL WEIGHTS FROM TENANT
========================================================= */

function getTenantWeights() {
  const tenant = getStoredTenant();
  ...
  if (
    tenant?.data &&
    Array.isArray(tenant.data.weights)
  ) {
    return tenant.data.weights;
  }

  if (Array.isArray(tenant.weights)) {
    return tenant.weights;
  }
...
/* =========================================================
   NORMALIZE WEIGHT GOAT ID
========================================================= */

function getWeightGoatId(record) {
  if (!record) return "";
...
/* =========================================================
   ...
   1. Scale records from tenant.data.weights
   2. Old goat.events records as fallback
========================================================= */
...
function buildWeightHistory(
  ...
  const goatId = getGoatId(goat);

  const goatTag = String(
    goat.tagNumber ||
      goat.tag ||
      goat.tagNo ||
      ""
  ...
  /* -------------------------------------------------------
     SCALE WEIGHT RECORDS
  ------------------------------------------------------- */

  const scaleRecords = allWeights
    .filter((record) => {
      const recordGoatId =
        getWeightGoatId(record);

      const recordTag = String(
        record.tagNumber ||
          record.tag ||
          record.tagNo ||
          ""
      ...
      /*
        Match using goatId OR tag number.
      */
  ...
  /* -------------------------------------------------------
     ...
     Avoid duplicate records if same id exists.
  ------------------------------------------------------- */
...
export default function ReportPage({
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      if (
        tenantGoats.length > 0 &&
        !selectedGoatId
      ) {
        setSelectedGoatId(
          tenantGoats[0].id ||
            tenantGoats[0]._id ||
            tenantGoats[0].tagNumber
        );
      }
    };
    ...
    /* -----------------------------------------------------
       IMPORTANT:
       WeighingScalePage dispatches this event after saving.
    ----------------------------------------------------- */
    ...
    const syncFromTenant = (
      ...
      setGoats(goatData);
      setAllWeights(weightData);

      /*
        If selected goat no longer exists,
        select first goat.
      */
  ...
  if (goats.length === 0) {
    return (
      <div className="report-page">
        <style>{createGlobalStyles()}</style>
          ...
          <section className="report-intro">
            ...
            <div>
              ...
              <p>
                ...
                Create a goat profile
                first to view weight
                reports.
              </p>
            </div>
      ...
      <main className="report-content">
        ...
        <section className="report-intro">
          ...
          <div>
            ...
            <p>
              View latest, previous
              and live scale weight
              records.
            </p>
            ...
            >
              ...
              {selectedGoat ? (
                <div className="selected-goat-content">
                  ...
                  <div className="mini-goat-avatar">
                    🐐
                  </div>
          ...
          <div className="print-area">
            ...
            <section className="weight-summary-grid">
              ...
              <SummaryCard
                ...
                value={
                  weightSummary.previous !==
                  null
                    ? `${weightSummary.previous.toFixed(
                        2
                      )} kg`
                    : "—"
                }
                ...
                null
                  ? `${
                      ...
                      0
                        ? "+"
                        : ""
                    }${weightSummary.percentage.toFixed(
                      1
                    )}%`
                  : "Need 2 checks"}
