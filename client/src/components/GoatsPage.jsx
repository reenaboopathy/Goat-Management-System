...
function persistTenantGoats(goats) {
  ...
  const updated = {
    ...
    data: {
      ...
      events: Array.isArray(existing.data?.events)
...
const ORIGIN_OPTIONS = [
  {
    ...
    icon: "🐐",
    title: "Born Here",
    subtitle: "Born on your farm",
  },
  {
    value: "Purchased",
    icon: <ShoppingBag size={20} />,
    title: "Purchased",
    subtitle: "Bought from another farm",
  },
  {
    value: "Other",
    icon: <Plus size={21} />,
...
const makeEvent = (type, title, extra = {}) => ({
  id: uid(),
  type,
  title,
  date: todayISO(),
...
function getBackendGoats(data) {
  return getApiArray(data, [
    "goats",
    "data",
    "results",
  ]).map(normalizeGoat);
...
export default function GoatsPageDemo({
  onBack,
}) {
  const [goatsList, setGoatsList] =
    useState([]);
  ...
  const [breedQuery, setBreedQuery] =
    useState("");
  ...
  async function loadGoats() {
    ...
    try {
      ...
      const backendGoats = getBackendGoats(response);

      setGoatsList(backendGoats);
      setBackendConnected(true);
      setSubscriptionBlocked(false);
      setSubscriptionMessage("");
      setApiError("");

      persistTenantGoats(backendGoats);
    } catch (error) {
      console.error("GET /api/goats failed:", error);

      // 402 means the backend IS connected, but farm access is blocked.
      if (isSubscriptionError(error)) {
        ...
        setFormOpen(false);
        setEditingGoat(null);
        setRemovingGoat(null);
        return;
      }

      // 401 means the backend is reachable, but the login/session is invalid.
      ...
      // status 0 is an actual network/backend connection problem.
      ...
      setApiError(
        error?.message ||
          "Unable to load goats from the backend server."
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const syncFromTenant = () => {
      // Backend is the source of truth.
      // Do not replace backend/subscription state with localStorage data.
    };

    const syncFromStorage = () => {
      // Intentionally ignored for goat data.
      // Goat data must come from the authenticated backend.
    };
  ...
  async function createGoat(goat) {
    const normalized = normalizeGoat(goat);
    ...
    try {
      const response = await apiRequest("/goats", {
        ...
        body: JSON.stringify({
          ...
          sireTagNumber: normalized.sireTagNumber,
          damTagNumber: normalized.damTagNumber,
          notes: normalized.notes,
          photo: normalized.photo,
          archived: false,
          dateOfEntry: normalized.dateOfEntry,
        }),
      });

      const created = normalizeGoat(
        response?.goat ||
          response?.data ||
          response
      );
      ...
      if (isSubscriptionError(error)) {
        ...
        setApiError(
          error?.message ||
            "Unable to create the goat."
        );
      }

      // IMPORTANT: never create a local-only goat when the backend rejects it.
      throw error;
    }
  }
  ...
  async function updateGoat(goat) {
    ...
    try {
      ...
      console.error("PUT /api/goats failed:", error);
      ...
      if (isSubscriptionError(error)) {
        ...
        setApiError(
          error?.message ||
            "Unable to update the goat."
        );
      }

      // IMPORTANT: do not update local state when backend update fails.
      throw error;
    }
  }
  ...
  async function deleteGoat(goatId) {
    try {
      await apiRequest(
        `/goats/${goatId}`,
        {
      ...
      if (isSubscriptionError(error)) {
        ...
        setApiError(
          error?.message ||
            "Unable to delete the goat."
        );
      }

      // IMPORTANT: never remove the goat locally if DELETE failed.
      throw error;
    }
  }
  ...
  async function archiveGoat(goatId) {
    const goat = goatsList.find(
      (g) =>
        String(g.id) === String(goatId)
    );
      ...
      <div className="gp-content">
        ...
        {subscriptionBlocked ? (
          <div className="gp-empty">
            ...
            >
              {subscriptionMessage ||
                "Your 14-day free trial has ended. Please renew your subscription to continue accessing your farm data."}
            </p>
  ...
  return (
    <div className="gp-goat-card">
      ...
      >
        ...
        >
          <div className="gp-goat-tag">
            #
            {goat.tagNumber ||
              "—"}
          </div>

          <h3 className="gp-goat-name">
            {goat.name ||
              "Unnamed Goat"}
          </h3>

          <div className="gp-goat-breed">
            {goat.breed ||
              "Breed not specified"}
          </div>

          <div className="gp-goat-stage">
            {goat.stage ||
              "Kid"}
          </div>
        </div>

        <div className="gp-goat-gender">
          {goat.gender || "—"}
        </div>
      </div>
      ...
      >
        <button
          type="button"
          className="gp-card-menu-btn"
          aria-label="Goat options"
          onClick={(e) => {
            e.stopPropagation();
            onToggleMenu();
          }}
        ...
        {menuOpen && (
          <>
            <div
              className="gp-card-menu-backdrop"
              onClick={onCloseMenu}
            />

            <div
              className="gp-card-menu"
              onClick={(e) =>
                e.stopPropagation()
              }
            >
...
function GoatDetailPage({
  ...
  async function addWeight(weight) {
    ...
    const difference =
      previousWeight != null
        ? numericWeight - previousWeight
        : 0;

    const weightEvent = makeEvent(
      "Weight",
      `Weight recorded: ${numericWeight} kg`,
      {
        previousWeight,
        difference,
      }
    ...
    try {
      ...
      // Only update the goat after the weight record was accepted.
      await updateGoat({
        weight: numericWeight,
      ...
      // updateGoat handles its own error state.
      // This catch ensures the modal does not silently close.
      throw error;
    }
  }
        ...
        <div className="gp-body">
          ...
          <div className="gp-panel">
            ...
            <div className="gp-empty-note">
              No offspring linked
              yet! To link one,
              edit the offspring's
              record and enter this
              goat's tag number in
              the Father's tag no
              field.
            </div>
          </div>
      ...
      >
        ...
        <Field label="Event Title">
          <input
            ...
            placeholder="e.g. Vaccination completed"
          />
        </Field>
          ...
          <section className="gp-form-card">
            ...
            <div className="gp-visual-field">
              ...
              <div className="gp-visual-wrap">
                <span className="gp-visual-prefix">
                  #
                </span>

                <input
                  className="gp-visual-input"
                  value={tagNumber}
                  onChange={(e) =>
                    setTagNumber(
                      e.target
        ...
        <section className="gp-form-card">
          ...
          <div className="gp-three-col">
            <div className="gp-timeline-field">
              <div className="gp-timeline-icon">
                <CalendarDays
                  size={18}
                />
              </div>
          ...
          <div className="gp-origin-grid">
            {ORIGIN_OPTIONS.map(
              (o) => (
                ...
                >
                  <div className="gp-origin-icon">
                    {o.icon}
                  </div>

                  <div>
                    <strong className="gp-origin-title">
                      {o.title}
                    </strong>

                    <span className="gp-origin-subtitle">
                      {
                        o.subtitle
                      }
                    </span>
...
/* =========================================================
   LIVE GOAT WEIGHT SCALE
========================================================= */

function GoatWeightScale({
  goat,
  onBack,
  onSave,
}) {
  ...
  /*
    ...
    - Manual reading works immediately.
    - Later ESP32/USB Serial can update `reading`.
  */
    ...
    <div className="gp-app">
      <div className="gp-scale-header">
        <div className="gp-scale-header-inner">
          ...
          <div className="gp-scale-connected">
            <Wifi size={14} />
            Scale Ready
          </div>
        </div>
